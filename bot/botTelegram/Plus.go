...
func Plus(db *sql.DB, name string, chatid int64) {
	var countNames int
	...
	if countNames == 0 {
		...
		for results.Next() {
			...
			if tag.Name == name && tag.Timedown > 3 && tag.Chatid == chatid {
				mes := Send(chatid, "@"+name+" рановато плюсик жмешь, ты в очереди на кз"+tag.Lvlkz+" будешь еще "+fmt.Sprintf("%dмин", tag.Timedown))
				go SendDelMessage5s(chatid, mes)
			} else if tag.Name == name && tag.Timedown <= 3 && tag.Chatid == chatid {
				mes := Send(chatid, "@"+name+" время обновлено ")
				go SendDelMessage5s(chatid, mes)
				_, err := db.Exec("update sborkztg set timedown = timedown + 30 where active = 0 AND name = ? AND chatid = ?", tag.Name, tag.Chatid)
				if err != nil {
					log.Println(err)
				}
				Queue(db, tag.Lvlkz, tag.Chatid)
			}
		}
	}
}
