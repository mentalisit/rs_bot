...
func handler(rawEvt interface{}) {
	...
	case *events.Message:
		...
		fmt.Println("434 чат получателя  ", evt.Info.MessageSource.Chat)
		fmt.Println("435 отправитель ", evt.Info.MessageSource.Sender.String())
		fmt.Println("436 отправитель PushName", evt.Info.PushName)
		text := *evt.Message.Conversation
		fmt.Println(437, text)
		if text != "" {
			wasend()
		}
...
func wasend() { //chatid,text string) {
	var args []string
	chatid := "380989033544-1616265986@g.us"
	text := "проверка"
